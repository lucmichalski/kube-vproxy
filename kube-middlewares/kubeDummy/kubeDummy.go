...
// KubeDummyMiiddleware struct holds configuration parameters and is used to
// serialize/deserialize the configuration from storage engines.
type KubeDummyMiddleware struct {
	Password string
	Username string
}

// KubeDummy middleware handler
type KubeDummyHandler struct {
	cfg  KubeDummyMiddleware
	next http.Handler
}

// This function will be called each time the request hits the location with this middleware activated
func (a *KubeDummyHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
...
func New(user, pass string) (*KubeDummyMiddleware, error) {
	return &KubeDummyMiddleware{Username: user, Password: pass}, nil
}
...
func (c *KubeDummyMiddleware) NewHandler(next http.Handler) (http.Handler, error) {
	return &KubeDummyHandler{next: next, cfg: *c}, nil
}

// String() will be called by loggers inside Vulcand and command line tool.
func (c *KubeDummyMiddleware) String() string {
...
func FromOther(c KubeDummyMiddleware) (plugin.Middleware, error) {
...
func CliFlags() []cli.Flag {
	return []cli.Flag{
		cli.StringFlag{"user, u", "", "Basic KubeDummy username", ""},
		cli.StringFlag{"pass, p", "", "Basic KubeDummy pass", ""},
	}
}