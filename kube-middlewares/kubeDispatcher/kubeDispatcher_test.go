...
type KubeDispatcherSuite struct {
}

var _ = Suite(&KubeDispatcherSuite{})

// One of the most important tests:
// Make sure the RateLimit spec is compatible and will be accepted by middleware registry
func (s *KubeDispatcherSuite) TestSpecIsOK(c *C) {
	c.Assert(plugin.NewRegistry().AddSpec(GetSpec()), IsNil)
}

func (s *KubeDispatcherSuite) TestNew(c *C) {
...
func (s *KubeDispatcherSuite) TestNewBadParams(c *C) {
	// Empty pass
	_, err := New("user", "")
	c.Assert(err, NotNil)
	// Empty user
...
func (s *KubeDispatcherSuite) TestFromOther(c *C) {
	a, err := New("user", "pass")
	c.Assert(a, NotNil)
	c.Assert(err, IsNil)
...
func (s *KubeDispatcherSuite) TestKubeDispatcherFromCli(c *C) {
	app := cli.NewApp()
	app.Name = "test"
	executed := false
	app.Action = func(ctx *cli.Context) {
		...
		c.Assert(out, NotNil)
		c.Assert(err, IsNil)
		a := out.(*KubeDispatcherMiddleware)
...
func (s *KubeDispatcherSuite) TestRequestSuccess(c *C) {
	a := &KubeDispatcherMiddleware{Username: "aladdin", Password: "open sesame"}
...
func (s *KubeDispatcherSuite) TestRequestBadPassword(c *C) {
	a := &KubeDispatcherMiddleware{Username: "aladdin", Password: "open sesame"}
