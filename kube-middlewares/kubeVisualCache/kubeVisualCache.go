...
// KubeVisualCacheMiddleware struct holds configuration parameters and is used to
// serialize/deserialize the configuration from storage engines.
type KubeVisualCacheMiddleware struct {
	Template       string
	Queue          string
	MarkerId       int
	BlippId        int
...
// KubeVisualCache middleware handler
type KubeVisualCacheHandler struct {
	cfg  KubeVisualCacheMiddleware
	next http.Handler
}
...
func (a *KubeVisualCacheHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {

	w.Header().Set("X-Middleware-DISPATCHER-START",  strconv.FormatInt(time.Now().UTC().UnixNano(), 10))
	if a.cfg.Chained == 0 {
		w.Header().Set("KubeVision_CACHE_LOG_SKIPPED_BY_CONFIG", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))
		log.Println("KubeVision_CACHE_LOG_SKIPPED_BY_CONFIG, ", "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))					    
	...
	if err != nil {
		w.Header().Set("KubeVision_CACHE_ERROR_NO_INPUT", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))
		log.Println("KubeVision_CACHE_ERROR_NO_INPUT:", err, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))			    
		a.next.ServeHTTP(w, r)
		return
	}
	...
	if err != nil {
		w.Header().Set("KubeVision_CACHE_ERROR_DECODE_INPUT_FORMAT", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))
		log.Println("KubeVision_CACHE_ERROR_DECODE_INPUT_FORMAT:", err, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))					    
		a.next.ServeHTTP(w, r)
		return
	}

	if formatImg != "jpeg" {	    
		w.Header().Set("KubeVision_CACHE_ERROR_INVALID_INPUT_FORMAT", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))
		log.Println("KubeVision_CACHE_ERROR_INVALID_INPUT_FORMAT:", err, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))	   
		a.next.ServeHTTP(w, r)
		return
	} else {
		w.Header().Set("KubeVision_CACHE_LOG_VALID_INPUT_FORMAT", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))
		log.Println("KubeVision_CACHE_LOG_VALID_INPUT_FORMAT:", err, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))	
	}

	if a.cfg.Debug == 1 {
		log.Println("KubeVision_CACHE_LOG_FORMAT_TYPE:", formatImg, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))	
		log.Println("KubeVision_CACHE_LOG_BLIPP_ID:", a.cfg.BlippId, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))	
		log.Println("KubeVision_CACHE_LOG_MARKER_ID:", a.cfg.MarkerId, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))	
		log.Println("KubeVision_CACHE_LOG_CONTEXT:", a.cfg.Context, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))	
		log.Println("KubeVision_CACHE_LOG_THUMB_WIDTH:", a.cfg.Width, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))	
		log.Println("KubeVision_CACHE_LOG_THUMB_HEIGHT:", a.cfg.Height, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))	
		log.Println("KubeVision_CACHE_LOG_LEARNING_MODE:", a.cfg.Learn, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))	
		log.Println("KubeVision_CACHE_LOG_CHAIN:", a.cfg.Queue, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))	
		log.Println("KubeVision_CACHE_LOG_CONTENT_TYPE:", contentType, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))	
	}
	...
	if a.cfg.Transformation != "" || (a.cfg.Width > 0 || a.cfg.Height > 0) {
		...
		if a.cfg.Debug == 1 {
			w.Header().Set("KubeVision_CACHE_LOG_IMG_PROCESSING_CHAIN_START", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))					
			w.Header().Set("KubeVision_CACHE_LOG_IMG_PROCESSING_LENCHAIN", strconv.Itoa(len(transformations)))
			log.Println("KubeVision_CACHE_LOG_IMG_PROCESSING_START:", "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10), "lenght:", len(transformations))
		}
		for _, transform := range transformations {
			cmds := strings.Split(string(transform), "=")
			if a.cfg.Debug == 1 {
				w.Header().Set("KubeVision_CACHE_LOG_IMG_PROCESSING", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))					
				w.Header().Set("KubeVision_CACHE_LOG_IMG_PROCESSING_CMD", cmds[0])
				log.Println("KubeVision_CACHE_LOG_IMG_PROCESSING_CMD:", cmds[0], "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))																				
			}
			if cmds[0] == "Blur" {
				sigma, err := strconv.ParseFloat(cmds[1], 64)
				w.Header().Set("KubeVision_CACHE_LOG_IMG_BLUR_SIGMA", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))
				log.Println("KubeVision_CACHE_LOG_IMG_BLUR_SIGMA:", sigma, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))	
				if err != nil {
					w.Header().Set("KubeVision_CACHE_ERROR_IMG_BLUR_SIGMA", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))
					log.Println("KubeVision_CACHE_ERROR_IMG_BLUR_SIGMA:", err, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))			    
					a.next.ServeHTTP(w, r)
					return
				}
			...
			if cmds[0] == "Sharpen" {
				sigma, err := strconv.ParseFloat(cmds[1], 64)
				w.Header().Set("KubeVision_CACHE_LOG_IMG_SHARPEN_SIGMA", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))
				log.Println("KubeVision_CACHE_LOG_IMG_SHARPEN_SIGMA:", sigma, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))	
				if err != nil {					
					w.Header().Set("KubeVision_CACHE_ERROR_IMG_SHARPEN_SIGMA", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))
					log.Println("KubeVision_CACHE_ERROR_IMG_SHARPEN_SIGMA:", err, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))
					a.next.ServeHTTP(w, r)
					return
				}
			...
			if cmds[0] == "AdjustGamma" {
				sigma, err := strconv.ParseFloat(cmds[1], 64)
				w.Header().Set("KubeVision_CACHE_LOG_IMG_ADJUST-GAMMA_SIGMA", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))
				log.Println("KubeVision_CACHE_LOG_IMG_ADJUST-GAMMA_SIGMA:", sigma, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))	
				if err != nil {
					w.Header().Set("KubeVision_CACHE_ERROR_IMG_ADJUST-GAMMA_SIGMA", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))															
					log.Println("KubeVision_CACHE_ERROR_IMG_ADJUST-GAMMA_SIGMA:", err, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))
					a.next.ServeHTTP(w, r)
					return
				}
			...
			if cmds[0] == "AdjustContrast" {
				sigma, err := strconv.ParseFloat(cmds[1], 64)
				if err != nil {
					w.Header().Set("KubeVision_CACHE_ERROR_IMG_ADJUST-CONTRAST_SIGMA", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))										
					log.Println("KubeVision_CACHE_ERROR_IMG_ADJUST-CONTRAST_SIGMA", err, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))
					a.next.ServeHTTP(w, r)
				}
			...
			if cmds[0] == "AdjustBrightness" {
				sigma, err := strconv.ParseFloat(cmds[1], 64)
				if err != nil {
					w.Header().Set("KubeVision_CACHE_ERROR_IMG_ADJUST-BRIGHTNESS_SIGMA", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))					
					log.Println("KubeVision_CACHE_ERROR_IMG_ADJUST-BRIGHTNESS_SIGMA:", err, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))
					a.next.ServeHTTP(w, r)
					return
				}
			...
			if cmds[0] == "AdjustSigmoid" {
				...
				if err != nil {
					w.Header().Set("KubeVision_CACHE_ERROR_IMG_ADJUST-SIGMOID_MIDPOINT", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))
					log.Println("KubeVision_CACHE_ERROR_IMG_ADJUST-SIGMOID_MIDPOINT:", err, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))
					a.next.ServeHTTP(w, r)
					return
				}
				factor, err := strconv.ParseFloat(cmds[2], 64)
				if err != nil {
					w.Header().Set("KubeVision_CACHE_ERROR_IMG_ADJUST-SIGMOID_FACTOR", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))
					log.Println("KubeVision_CACHE_ERROR_IMG_ADJUST-SIGMOID_FACTOR", err, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))
					a.next.ServeHTTP(w, r)
					return
				}
			...
			if cmds[0] == "Crop" {
				...
				if err != nil {					
					w.Header().Set("KubeVision_CACHE_ERROR_IMG_CROP_X0", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))					
					log.Println("KubeVision_CACHE_ERROR_IMG_CROP_X0; ", err, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))
					a.next.ServeHTTP(w, r)
					return
				}
				y0, err := strconv.ParseInt(cmds[2], 0, 32)
				if err != nil {
					w.Header().Set("KubeVision_CACHE_ERROR_IMG_CROP_Y0", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))					
					log.Println("KubeVision_CACHE_ERROR_IMG_CROP_Y0; ", err, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))
					a.next.ServeHTTP(w, r)
					return
				}
				x1, err := strconv.ParseInt(cmds[3], 0, 32)
				if err != nil {					
					w.Header().Set("KubeVision_CACHE_ERROR_IMG_CROP_X1", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))					
					log.Println("KubeVision_CACHE_ERROR_IMG_CROP_X1; ", err, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))										
					a.next.ServeHTTP(w, r)
					return
				}
				y1, err := strconv.ParseInt(cmds[4], 0, 32)
				if err != nil {
					w.Header().Set("KubeVision_CACHE_ERROR_IMG_CROP_Y1", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))					
					log.Println("KubeVision_CACHE_ERROR_IMG_CROP_Y1; ", err, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))																				
					a.next.ServeHTTP(w, r)
					return
				}
			...
			if cmds[0] == "CropCenter" {
				...
				if err != nil {
					w.Header().Set("KubeVision_CACHE_ERROR_IMG_CROP-CENTER_WIDTH", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))					
					log.Println("KubeVision_CACHE_ERROR_IMG_CROP-CENTER_WIDTH; ", err, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))																				
					a.next.ServeHTTP(w, r)
					return
				}
				height, err := strconv.ParseInt(cmds[1], 0, 32)
				if err != nil {
					w.Header().Set("KubeVision_CACHE_ERROR_IMG_CROP-CENTER_HEIGHT", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))					
					log.Println("KubeVision_CACHE_ERROR_IMG_CROP-CENTER_HEIGHT; ", err, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))																				
					a.next.ServeHTTP(w, r)
					return
				}
		...
		w.Header().Set("KubeVision_CACHE_LOG_IMG_PROCESSING_CHAIN_END", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))					
	}

	buf := bytes.NewBuffer(nil)
	if err := jpeg.Encode(buf, dstImage, nil); err != nil {
		w.Header().Set("KubeVision_CACHE_ERROR_JPEGENCODE", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))					
		log.Println("KubeVision_CACHE_ERROR_JPEGENCODE; ", err, "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))																				
		a.next.ServeHTTP(w, r)
		return
	}
	...
	if a.cfg.Learn == 1 {
		...
		if a.cfg.Debug == 1 {
			w.Header().Set("KubeVision_CACHE_LOG_LEARNING_MODE_ACTIVATED", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))					
			log.Println("KubeVision_CACHE_LOG_LEARNING_MODE_ACTIVATED; ", "at: ", strconv.FormatInt(time.Now().UTC().UnixNano(), 10))
		}
	}
...
func (c *KubeVisualCacheMiddleware) NewHandler(next http.Handler) (http.Handler, error) {

	return &KubeVisualCacheHandler{next: next, cfg: *c}, nil
}

// String() will be called by loggers inside Kube-Vproxy and command line tool.
func (c *KubeVisualCacheMiddleware) String() string {
...
func FromOther(c KubeVisualCacheMiddleware) (plugin.Middleware, error) {
