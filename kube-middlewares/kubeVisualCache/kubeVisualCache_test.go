...
//
// Luc Michalski - 2015
// Kube Vision - kube Dispatcher (unit tests to be done)
//
...
type KubeVisualCacheSuite struct {
}

var _ = Suite(&KubeVisualCacheSuite{})

// One of the most important tests:
// Make sure the RateLimit spec is compatible and will be accepted by middleware registry
func (s *KubeVisualCacheSuite) TestSpecIsOK(c *C) {
	c.Assert(plugin.NewRegistry().AddSpec(GetSpec()), IsNil)
}

func (s *KubeVisualCacheSuite) TestNew(c *C) {
...
func (s *KubeVisualCacheSuite) TestNewBadParams(c *C) {
	// Empty pass
	_, err := New("user", "")
	c.Assert(err, NotNil)
	// Empty user
...
func (s *KubeVisualCacheSuite) TestFromOther(c *C) {
	a, err := New("user", "pass")
	c.Assert(a, NotNil)
	c.Assert(err, IsNil)
...
func (s *KubeVisualCacheSuite) TestKubeVisualCacheFromCli(c *C) {
	app := cli.NewApp()
	app.Name = "test"
	executed := false
	app.Action = func(ctx *cli.Context) {
		...
		c.Assert(out, NotNil)
		c.Assert(err, IsNil)
		a := out.(*KubeVisualCacheMiddleware)
...
func (s *KubeVisualCacheSuite) TestRequestSuccess(c *C) {
	a := &KubeVisualCacheMiddleware{Username: "aladdin", Password: "open sesame"}
...
func (s *KubeVisualCacheSuite) TestRequestBadPassword(c *C) {
	a := &KubeVisualCacheMiddleware{Username: "aladdin", Password: "open sesame"}
